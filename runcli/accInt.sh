...
export TEST_ECR_CLI_UP=$(echo $TEST_ECR_CLI | awk '{print toupper($0)}')
export TEST_ECR_ACCESS_KEY_ID=$(eval echo "$"$TEST_ECR_CLI_UP"_INTEGRATION_ECR_ACCESS_KEY_ID")
export TEST_ECR_INTEGRATION_SECRET_ACCESS_KEY=$(eval echo "$"$TEST_ECR_CLI_UP"_INTEGRATION_SECRET_ACCESS_KEY")
echo TEST_ECR_ACCESS_KEY_ID=${#TEST_ECR_ACCESS_KEY_ID}
echo TEST_ECR_INTEGRATIONS_SECRET_ACCESS_KEY=${#TEST_AWS_INTEGRATION_ECR_SECRET_ACCESS_KEY}
...
export TEST_GCR_INTEGRATION_URL=$(eval echo "$"$TEST_GCR_CLI_UP"_INTEGRATION_URL")
echo TEST_GCR_INTEGRATION_URL=$TEST_GCR_INTEGRATION_URL
...
export TEST_QUAY_INTEGRATION_USERNAME=$(eval echo "$"$TEST_AWS_CLI_UP"_INTEGRATION_USERNAME")
export TEST_QUAY_INTEGRATION_PASSWORD=$(eval echo "$"$TEST_AWS_CLI_UP"_INTEGRATION_PASSWORD")
